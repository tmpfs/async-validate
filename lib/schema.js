...
function Schema(rules, opts) {
  ...
  if(typeof rules !== 'object' && typeof rules !== 'function') {
...
function validate(source, options, cb) {
  ...
  console.dir('assign to rules : ' + series.length);
  ...
  // TODO: re-instate specific key iteration
  //keys = options.keys || Object.keys(this.rules.fields);

  // iterate list data
  func(series, function(rule, callback) {

    console.log('validate field %s with value: %j', rule.field, rule.value);
    ...
    console.log('(%s) validate %s', series.length, rule.field);
    //console.dir(rule);
