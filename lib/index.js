...
Schema.prototype.validate = function(source, options, callback) {
  ...
  var create = function(rule, value, source) {
  ...
  for(j = 0;j < keys.length;j++) {
    ...
    for(i = 0;i < arr.length;i++) {
      ...
      series.push(create(rule, value, source));
      if((rule.type == 'object' || rule.type == 'array')
          && typeof(rule.fields) == 'object') {
        //series.push(this.getDeepValidator(rule, value, options));
...
Schema.prototype.getDeepValidator = function(series, rule, value, options) {
  ...
  //if((rule.type == 'object' || rule.type == 'array')
    ...
    //if(value === undefined) {
      ...
      //return false;
    //}
    //console.log("GOT RULE FIELDS: %j", rule.fields);
    //// handle nested rule validation
    //return function(rule, value, callback, source) {
      //var schema = new Schema(rule.fields);
      //schema.validate(value, rule.options || options, function(errors, fields) {
        //callback(errors);
      //});
    //}
  //}
